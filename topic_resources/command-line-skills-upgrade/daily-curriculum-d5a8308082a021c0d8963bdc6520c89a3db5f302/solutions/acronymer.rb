...
class Acronym
  ...
  def initialize
    @word_bank = ["Aizoon", "Boone", "Demophoon", "Dioon", "Diplozoon", "Gaboon", "Gadswoons", "Joon", "Kokoona", "Leucocytozoon", "Roosevelt", "Rooseveltian", "Toona", "Walloon", "aboon", "actinozoon", "afternoon", "afternoons", "akhoond", "aloose",
...
# - Capitalize and concatenate each word to form the full sentence
  def acronymer(bag)
    characters = bag.downcase.chars
    matches = []
    characters.each do |letter|
      # find all matches instead of one
      # use sample to get a random word
      all_matches = @word_bank.find_all {|word| word[0].downcase == letter }
